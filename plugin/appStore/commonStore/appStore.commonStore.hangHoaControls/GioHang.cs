    ...
    public class GioHang : docPlugUI
    {
        ...
        public override void KhoiTao(SqlConnection con)
        {
            ...
            if (Security.IsAuthenticated())
                mem = MemberDal.SelectByUser(Security.Username);
            var tygia = DanhMucDal.SelectByMa("TYGIA", con);
            sb.AppendFormat(Lib.GetResource(Assembly.GetExecutingAssembly(), "GioHang.htm")
                ,mem.Ten
                , mem.Email
                , mem.Mobile
                , mem.DiaChi
            ...
            sb.Append(Lib.GetResource(Assembly.GetExecutingAssembly(), "GioHang_tmpl.htm"));
            #region cart
            #endregion

            Html = ResourceHelper(sb.ToString(),Lang);
            base.KhoiTao(con);
        }
