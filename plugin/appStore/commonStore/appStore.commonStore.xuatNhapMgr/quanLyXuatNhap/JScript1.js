var quanLyXuatNhapFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=appStore.commonStore.xuatNhapMgr.quanLyXuatNhap.Class1, appStore.commonStore.xuatNhapMgr'; },
    urlDefault1: function () { return adm.urlDefault1 + '&act=loadPlug&rqPlug=appStore.commonStore.xuatNhapMgr.quanLyXuatNhap.Class1, appStore.commonStore.xuatNhapMgr'; },
    setup: function () { },
    loadgrid: function () {
        adm.styleButton();
        var DMID = $('.mdl-head-HangHoaFilterDanhMuc');
        var q = $('.mdl-head-search-hangHoaMgr');
        $('#quanLyXuatNhapMdl-List').jqGrid({
            url: quanLyXuatNhapFn.urlDefault().toString() + '&subAct=get',
            height: '400',
            datatype: 'json',
            loadui: 'disable',
            ...
            sortorder: 'desc',
            pager: jQuery('#quanLyXuatNhapMdl-Pager'),
            rowNum: 10,
    ...
    XuatNhapChiTietItemFn: function (_obj) {
        ...
        Xoa.click(function () {
            ...
            obj.fadeOut('200');
            $.ajax({
                url: quanLyXuatNhapFn.urlDefault().toString(),
                dataType: 'script',
                data: {
                    subAct: 'XoaXNChiTiet',
                    'ID': id
        ...
        adm.regType(typeof (hangHoaMgrFn), 'appStore.commonStore.hangHoaMgr.Class1, appStore.commonStore.hangHoaMgr', function () {
            hangHoaMgrFn.autoCompleteByQ(HangHoa, function (event, ui) {
                ...
                if ($(cItem).length < 1) {
                    ...
                    HangHoa.focus();
                    HangHoa.val('');
                    quanLyXuatNhapFn.XuatNhapChiTietItemFn(cItem);
                    quanLyXuatNhapFn.recount();
                    quanLyXuatNhapFn.updateXNCTItem(cItem);
                }
                return false;
            });
    ...
    refresh: function (grid) {
        if (typeof (grid) == 'undefined') grid == '#quanLyXuatNhapMdl-List';
        var timerSearch;
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $(grid).jqGrid('setGridParam', { url: quanLyXuatNhapFn.urlDefault().toString() + '&subAct=get' }).trigger('reloadGrid');
        }, 500);
    },
    edit: function (grid) {
        var s = '';
    ...
    add: function (_newDlg, fn, obj, openFn) {
        quanLyXuatNhapFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        quanLyXuatNhapFn.save(false, function (_ID) {
    ...
    loc: function () {
        quanLyXuatNhapFn.loadHtml(function () {
            var newDlg = $('#quanLyXuatNhapMdl-dlgFilter');
            $(newDlg).dialog({
                title: 'Lọc',
                width: 600,
                modal: true,
                buttons: {
                    'Tìm': function () {
                        ...
                        $('#quanLyXuatNhapMdl-List').jqGrid('setGridParam', { url: quanLyXuatNhapFn.urlDefault().toString() + '&subAct=get&DM_ID=' + _DM_ID + '&KH_ID=' + _KH_ID + '&TuNgay=' + _TuNgay + '&DenNgay=' + _DenNgay }).trigger('reloadGrid');

                    },
                    'Nhập lại': function () {
                        quanLyXuatNhapFn.locClearform();
                    },
                    'Đóng': function () {
                        $(newDlg).dialog('close');
                    }
                ...
                open: function () {
                    adm.styleButton();
                    quanLyXuatNhapFn.locClearform();
                    quanLyXuatNhapFn.locPopfn(newDlg);
                }
            });
        });
    },
    ...
    del: function (grid) {
        ...
        if (s == '') {
            ...
            if (confirm('Bạn có chắc chắn xóa?')) {// Xác nhận xem có xóa hay không
                $.ajax({
                    url: quanLyXuatNhapFn.urlDefault().toString() + '&subAct=del',
                    dataType: 'script',
                    data: { 'ID': s },
                    success: function (dt) {
    ...
    search: function () {
        ...
        if (timerSearch) clearTimeout(timerSearch);
        timerSearch = setTimeout(function () {
            $('#quanLyXuatNhapMdl-List').jqGrid('setGridParam', { url: quanLyXuatNhapFn.urlDefault().toString() + '&subAct=get&DM_ID=' + _DMID + '&q=' + q.val() }).trigger('reloadGrid');
        }, 500);
    },
    draff: function (fn) {
        $.ajax({
    ...
    loadHtml: function (fn) {
        var dlg = $('#quanLyXuatNhapMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("appStore.commonStore.xuatNhapMgr.quanLyXuatNhap.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
