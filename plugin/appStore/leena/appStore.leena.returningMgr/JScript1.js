var returningMgrFn = {
    urlDefault: adm.urlDefault + '&act=loadPlug&rqPlug=appStore.leena.returningMgr.Class1, appStore.leena.returningMgr',
    setup: function () {
    },
    loadgrid: function () {
        adm.loading('Đang lấy dữ liệu');
        adm.styleButton();
        $('#returningMgrMdl-List').jqGrid({
            url: returningMgrFn.urlDefault + '&subAct=get',
            datatype: 'json',
            ...
            pager: jQuery('#returningMgrMdl-Pager'),
            onSelectRow: function (rowid) {
                var treedata = $("#returningMgrMdl-List").jqGrid('getRowData', rowid);
            },
    ...
    search: function () {
        var timerSearch;
        var searchTxt = $('.mdl-head-search-returningMgrMdl');
        var __q = $(searchTxt).val();
        var DM_ID = $('.mdl-head-returningMgrMdlFilterByDm');
        var _dm_id = '';
        if (DM_ID.val() != '') {
        ...
        timerSearch = setTimeout(function () {
            $('#returningMgrMdl-List').jqGrid('setGridParam', { url: returningMgrFn.urlDefault
             + '&q=' + __q
             + '&subAct=get'
             + '&_DM_ID=' + _dm_id
            }).trigger("reloadGrid");
    ...
    edit: function () {
        var s = '';
        if (jQuery('#returningMgrMdl-List').jqGrid('getGridParam', 'selrow') != null) {
            s = jQuery('#returningMgrMdl-List').jqGrid('getGridParam', 'selrow').toString();
        }
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                returningMgrFn.loadHtml(function () {
                    var newDlg = $('#returningMgrMdl-dlgNew');
                    $(newDlg).dialog({
                        title: 'Sửa',
                        width: 900,
                        buttons: {
                            'Lưu': function () {
                                returningMgrFn.save(function () { returningMgrFn.clearform(); });
                            },
                            'Lưu và đóng': function () {
                                returningMgrFn.save(function () { returningMgrFn.clearform(); $(newDlg).dialog('close'); });
                            },
                            'Đóng': function () {
                                $(newDlg).dialog('close');
                            }
                        ...
                        open: function () {
                            ...
                            adm.styleButton();
                            returningMgrFn.clearform();
                            returningMgrFn.popfn();
                            $.ajax({
                                url: returningMgrFn.urlDefault + '&subAct=edit',
                                dataType: 'script',
                                type: 'POST',
                                data: {
                                    'ID': s
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                if (confirm('Bạn có chắc chắn xóa hỏi đáp này?')) {
                    $.ajax({
                        url: returningMgrFn.urlDefault + '&subAct=del',
                        dataType: 'script',
                        data: {
                            'ID': s
                        },
                        success: function (dt) {
                            jQuery('#returningMgrMdl-List').trigger('reloadGrid');
                        }
                    });
                }
            }
    ...
    save: function (fn) {
        ...
        $.ajax({
            url: returningMgrFn.urlDefault + '&subAct=save',
            dataType: 'script',
            type: 'POST',
            data: {
                'ID': _ID,
    ...
    add: function () {
        returningMgrFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        returningMgrFn.save(function () {
                            returningMgrFn.clearform();
                        }, '#returningMgrMdl-List');
                    },
                    'Lưu và đóng': function () {
                        returningMgrFn.save(function () {
                            $(newDlg).dialog('close');
    ...
    loadHtml: function (fn) {
        var dlg = $('#returningMgrMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("appStore.leena.returningMgr.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
