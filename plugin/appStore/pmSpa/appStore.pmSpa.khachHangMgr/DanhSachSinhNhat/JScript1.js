var DanhSachSinhNhatFn = {
    urlDefault: function () { return adm.urlDefault + '&act=loadPlug&rqPlug=appStore.pmSpa.khachHangMgr.DanhSachSinhNhat.Class1, appStore.pmSpa.khachHangMgr'; },
    EditForm: '#DanhSachSinhNhatMdl-dlgNew',
    setup: function () { },
    loadgrid: function () {
        adm.styleButton();
        adm.loading('Đang lấy dữ liệu');

        var filterKhachHangByNguonGoc = $('.filterKhachHangByNguonGoc-NS');
        var filterKhachHangByKhuVuc = $('.filterKhachHangByKhuVuc-NS');
        var q = $('.mdl-head-search-DanhSachSinhNhatMdl');
        adm.watermark(filterKhachHangByKhuVuc, 'Gõ khu vực', function () {
            DanhSachSinhNhatFn.search();
        });
        ...
        $('#DanhSachSinhNhatMdl-List').jqGrid({
            url: DanhSachSinhNhatFn.urlDefault().toString() + '&subAct=get&Lang=' + Lang,
            height: '1000',
            datatype: 'json',
            loadui: 'disable',
            colNames: ['ID', 'Mã', 'Tên', 'Mobile', 'Email', 'Giới tính', 'Ym', 'Ngày sinh', 'Khu vực', 'Nguồn gốc','Tuổi', 'Lịch sử'],
            colModel: [
                ...
                { name: 'Tuoi', width: 5, resizable: true, sortable: true },
            ...
            sortname: 'NgayTao',
            sortorder: 'asc',
            pager: jQuery('#DanhSachSinhNhatMdl-Pager'),
            rowNum: 10,
    ...
    edit: function (grid) {
        var s = '';
        if (typeof (grid) == 'undefined') grid == '#DanhSachSinhNhatMdl-List';
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                DanhSachSinhNhatFn.loadHtml(function () {
                    var newDlg = $('#DanhSachSinhNhatMdl-dlgNew');
                    $(newDlg).dialog({
                        title: 'Sửa',
                        width: 900,
                        buttons: {
                        ...
                        beforeClose: function () {
                            DanhSachSinhNhatFn.clearform();
                        },
                        open: function () {
                            adm.loading('Đang nạp dữ liệu');
                            adm.styleButton();
                            DanhSachSinhNhatFn.clearform();
                            DanhSachSinhNhatFn.popfn();
                            $.ajax({
                                url: DanhSachSinhNhatFn.urlDefault().toString() + '&subAct=edit',
                                data: {
                                    'ID': s
                                },
    ...
    add: function (fn) {
        DanhSachSinhNhatFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        DanhSachSinhNhatFn.save(false, function (_ID, _Ten) {
                            if (typeof (fn) == 'function') {
                                fn(_ID, _Ten);
                            }
                            DanhSachSinhNhatFn.clearform();
                            DanhSachSinhNhatFn.draff(function () {

                            });
                        }, '#DanhSachSinhNhatMdl-List');
                    },
        ...
        if (s == '') {
            ...
            if (s.indexOf(',') != -1) {
                ...
                if (confirm('Bạn có chắc chắn xóa khách hàng này?')) {// Xác nhận xem có xóa hay không
                    $.ajax({
                        url: DanhSachSinhNhatFn.urlDefault().toString() + '&subAct=del',
                        dataType: 'script',
                        data: { 'ID': s },
                        success: function (dt) {
    ...
    save: function (validate, fn, grid) {
        ...
        $.ajax({
            url: DanhSachSinhNhatFn.urlDefault().toString(),
            type: 'POST',
            data: {
                'subAct': 'save',
                ID: _ID,
    ...
    sendmail: function () {
        var s = '';
        s = jQuery("#DanhSachSinhNhatMdl-List").jqGrid('getGridParam', 'selarrrow').toString();
        DanhSachSinhNhatFn.loadHtml(function () {
            var emailDlg = $('#DanhSachSinhNhatMdl-dlgEmail');
            $(emailDlg).dialog({
                ...
                open: function () {
                    ...
                    if (s != '') {
                        ...
                        for (j = 0; j < ss.length; j++) {
                            if (ss[j] != '') {
                                var treedata = $("#DanhSachSinhNhatMdl-List").jqGrid('getRowData', ss[j]);
                    ...
                    DanhSachSinhNhatFn.autoCompleteSearch(EmailTo, function (e, ui) {
    ...
    search: function () {
        ...
        timerSearch = setTimeout(function () {
            $('#DanhSachSinhNhatMdl-List').jqGrid('setGridParam', {
                url: DanhSachSinhNhatFn.urlDefault().toString() + '&subAct=get&NguonGoc_ID=' + NguonGoc_ID + '&KhuVuc_ID=' + KhuVuc_ID + '&q=' + _q
            }).trigger('reloadGrid');
        }, 1000);
    },
    popfn: function () {
        ...
        DanhSachSinhNhatFn.autoCompleteSearch(NguoiGioiThieu, function (event, ui) {
    ...
    draff: function (fn) {
        var newDlg = $(DanhSachSinhNhatFn.EditForm);
        $.ajax({
            url: DanhSachSinhNhatFn.urlDefault().toString(),
            type: 'POST',
            dataType: 'script',
    ...
    loadHtml: function (fn) {
        var dlg = $('#DanhSachSinhNhatMdl-dlgNew');
        if ($(dlg).length < 1) {
            adm.loading('Dựng from');
            $.ajax({
                url: '<%=WebResource("appStore.pmSpa.khachHangMgr.DanhSachSinhNhat.htm.htm")%>',
                success: function (dt) {
                    adm.loading(null);
                    $('body').append(dt);
                    if (typeof (fn) == 'function') {
