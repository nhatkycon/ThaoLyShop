...
[assembly: WebResource("appStore.pmSpa.khachHangMgr.thongKeKhachHang.htm.htm", "text/html", PerformSubstitution = true)]
[assembly: WebResource("appStore.pmSpa.khachHangMgr.thongKeKhachHang.JScript1.js", "text/javascript", PerformSubstitution = true)]

namespace appStore.pmSpa.khachHangMgr.thongKeKhachHang
{
    public class Class1 : docPlugUI
    {
        ...
        protected override void Render(HtmlTextWriter writer)
        {
            ...
            switch (subAct)
            {
                case "get":
                    #region get
                    var pagerGet = KhachHangDal.pagerDoanhSo("", false, "a.KH_" + jgrsidx + " " + jgrsord, _q, Convert.ToInt32(jgRows), KhuVuc_ID, NguonGoc_ID, TuNgay,DenNgay);
                    double DoanhSo = 0;
                    foreach (var item in pagerGet.List)
                    {
                        ListRow.Add(new jgridRow(item.ID.ToString(), new string[] { 
                            ...
                            , item.KhuVuc_Ten
                            , item.NguonGoc_Ten                          
                            , Lib.TienVietNam(item.DoanhSo)
                        }));
                        DoanhSo += item.DoanhSo;
                    }
                    var uData = new Dictionary<string, object>();
                    uData.Add("NguonGoc_ID", "Tổng");
                    uData.Add("DoanhSo", Lib.TienVietNam(DoanhSo));
                ...
                case "scpt":
                    #region Nạp js
                    sb.AppendFormat(@"{0}"
                        , cs.GetWebResourceUrl(typeof(Class1), "appStore.pmSpa.khachHangMgr.thongKeKhachHang.JScript1.js"));
                    break;
                    #endregion
                default:
                    #region default
                    {
                        ...
                        sb.Append(Lib.GetResource(Assembly.GetExecutingAssembly(), "thongKeKhachHang.mdl.htm"));
                        sb.AppendFormat(@"<script>$.getScript('{0}',function(){1});</script>"
                            , cs.GetWebResourceUrl(typeof(Class1), "appStore.pmSpa.khachHangMgr.thongKeKhachHang.JScript1.js")
                            , "{thongKeKhachHangFn.loadgrid();}");
