var SuKienAllFn = {
    ...
    EditForm: '#DeskTopSuKienKhachHangMdl-newDlg',
    AddLichProxy: function () {
        if (!loggedIn) {
            alert('Bạn cần đăng nhập'); return false;
        }
        SuKienAllFn.AddLich(function(dt) {
            var myUi = eval(dt);
            var skList = $('.suKien-list');
            $('#suKienDesktop-item').tmpl(myUi).prependTo(skList);
        });
    },
    AddLich: function (fn) {
        SuKienAllFn.loadHtml(function () {
            var newDlg = $(SuKienAllFn.EditForm);
            $(newDlg).dialog({
                title: 'Thêm lịch',
                width: 600,
                modal: true,
                buttons: {
                    'Lưu': function () {
                        SuKienAllFn.save({}, function (dt) {
                            if(typeof (fn) =="function") {
                                fn(dt);
                            }
                            SuKienAllFn.clearform();
                        });
                    },
    ...
    get: function () {
        ...
        $.ajax({
            url: SuKienAllFn.urlDefault().toString(),
            data: { 'subAct': 'get' },
            type: 'POST',
            dataType: 'script',
            success: function (dt) {
    ...
    edit: function (id) {
        ...
        SuKienAllFn.view(id, {}, function (dt) {
            var myUi = eval(dt);
            var cItem = $('.ssuKien-item[id="' + id + '"]');
            var newItem = $('#suKienDesktop-item').tmpl(myUi).html();
            $(cItem).html(newItem);
        });
    },
    view: function (id, obj1, fn) {
        if (!loggedIn) {
            alert('Bạn cần đăng nhập'); return false;
        }
        SuKienAllFn.loadHtml(function () {
            ...
            $(newDlg).dialog({
                ...
                buttons: {
                    'Lưu': function () {
                        SuKienAllFn.save(obj1, function (dt) {
                            if (typeof (fn) == "function") {
                                fn(dt);
                            }
                            SuKienAllFn.clearform();
    ...
    save: function (obj, fn) {
        ...
        if (_KH_ID == null) { err = 'Chọn khách hàng<br/>'; }
        if (err != '') {
    ...
    popfn: function () {
        ...
        SuKienAllFn.getGio(Gio, function (e, ui) {
        });
    ...
    timKhachHangPopup: function () {
        SuKienAllFn.loadHtml(function () {
            var newDlg = $(SuKienAllFn.TimKiemForm);
            $(newDlg).dialog({
                title: 'Tìm kiếm',
                width: 400,
                modal: true,
                ...
                open: function () {
                    common.styleButton();
                    SuKienAllFn.timKhachHangPopupFn();
                }
            });
        });
    },
    loadHtml: function (fn) {
        var dlg = $(SuKienAllFn.EditForm);
        if ($(dlg).length < 1) {
