...
namespace appStore.rssAppStore.advApp
{
    public class chiTiet_kieu1 : PlugUI
    {
...
<div class=""box2"" style=""{0}"">
        <div class=""box-body"">", Css, Ten, Header_Url);
            ...
            var List = docsoft.entities.QuangCaoDal.SelectByDanhMuc(con, Ma, Convert.ToInt32(Top));
            if (List.Count > 0)
            {
                foreach (docsoft.entities.QuangCao item in List)
                {
                    sb.AppendFormat(docsoft.entities.QuangCaoDal.formatAdvChiTietKieu1(item, domain));
                }
            }
            else
            {
                sb.AppendFormat(@"Chưa có quảng cáo khu vực {0}", Ma);
            }
            sb.AppendFormat(@"
        </div>
</div>");
