...
{
    public class home_danhMucStyle2 : PlugUI
    {
...
<div class=""box2"" style=""{0}"">
    <div class=""box-header"">
        ...
        <div class=""box-body"">", Css, Ten, Header_Url);
            ...
            List<Tin> List1 = TinDal.SelectMoiDanhMuc(con, Convert.ToInt32(Top), Ma);
            int i = 0;
            #region tin noi bat
            sb.AppendFormat(@"<div class=""danhMucStyle2-leftPnl"">");
            foreach (Tin item in List)
            {
                switch (i)
                {
                    case 0:
                        sb.Append(TinDal.formatDanhMucStyle21Big(item, domain));
                        break;
                    default:
                        sb.Append(TinDal.formatDanhMucStyle21Tiny(item, domain));
                        break;
                }
                i++;
            }
            ...
            #region tin noi bat
            i = 0;
            sb.AppendFormat(@"<div class=""danhMucStyle2-rightPnl"">");
            foreach (Tin item in List1)
            {
                switch (i)
                {
                    case 0:
                        sb.Append(TinDal.formatDanhMucStyle22Big(item, domain));
                        break;
                    default:
                        sb.Append(TinDal.formatDanhMucStyle22Tiny(item, domain));
                        break;
                }
                i++;
            }
