...
{
    public class home_danhMucStyle6 : PlugUI
    {
...
<div class=""box-header-task"" _Top=""{0}"" _Type=""{1}"">
    ...
    <a href=""javascript:;"" class=""box-header-task-icon box-header-task-minus""></a>
</div>", Top, typeof(home_danhMucStyle6).AssemblyQualifiedName) : "");
            ...
            foreach (Tin item in List)
            {
                switch (i) { 
                    case 0:
                        sb.Append(TinDal.formatDanhMucStyle6Big(item, domain));
                        break;
                    default:
                        sb.Append(TinDal.formatDanhMucStyle6Tiny(item, domain));
                        break;
                }
                i++;
            }
